...
def GD(trainData, trainHeadNew, step, eps):
    ...
    while True:
        for j in xrange(row):
            e0 = 0
            for k in xrange(col):
                e0 = e0 + linCoef[k] * linCoef[k]
            s1 = math.sqrt(e0)

            linCoef = linCoef - step * (LR(trainData[j], linCoef) - trainHeadNew[j]) * trainData[j]
    ...
    for i in range(3):
        ...
        for j in range(len(trainHeadNew)):
            if (trainHeadNew[j] == i):
                ...
                trainHeadNew[j] = 0
        linCoef[i] = GD(trainData, trainHeadNew, 10, 0.005)
    return linCoef
