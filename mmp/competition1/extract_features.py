...
def extract_match_features(match, time_point=None):
    ...
    for player_index, player in enumerate(match['players']):
        player_id = ('r%d' % (player_index+1)) if player_index < 5 \
                else ('d%d' % (player_index-4))

        feats += [
            (player_id + '_hero', player['hero_id']),
            (player_id + '_level', max([0] + [entry['level'] \
                    for entry in filter_events(player['ability_upgrades'], \
                    time_point)])),
            (player_id + '_xp', last_value(player['xp_t'], times, time_point)),
            (player_id + '_gold', last_value(player['gold_t'], times, \
                    time_point)),
            (player_id + '_lh', last_value(player['lh_t'], times, time_point)),
            (player_id + '_kills', len(filter_events(player['kills_log'], \
                    time_point))),
            (player_id + '_deaths', len([
                    1
                    for other_player in match['players']
                    for event in filter_events(other_player['kills_log'], \
                            time_point)
                    if event['player'] == player_index
                ])),
            (player_id + '_items', len(filter_events(player['purchase_log'], \
                    time_point))),
        ]

    # first blood
    first_blood_objectives = filter_events([obj for obj \
            in match['objectives'] if obj['type'] == 'firstblood'], time_point)
    fb = first_blood_objectives[0] if len(first_blood_objectives) > 0 else {}
    feats += [
        ('first_blood_time', fb.get('time')),
        ('first_blood_team', int(fb['player1'] >= 5) if fb.get('player1') \
    ...
    for team, team_players in (('radiant', radiant_players), \
            ('dire', dire_players)):
        ...
        feats += [
            ('%s_first_ward_time' % team, min([entry['time'] for entry \
                    in team_wards]) if len(team_wards) > 0 else None),
        ]
    ...
    for match in iterate_matches(matches_filename):
        ...
        for key, value in features.items():
            if value:
                ...
                df[key].append(0)
    df = pandas.DataFrame.from_records(df).ix[:, fields] \
            .set_index('match_id').sort_index()
    return df


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
            description='Extract features from matches data')
